...
function processDuplicateFree(list, callback) {
  return callback(list.filter((entry, index) => list.indexOf(entry) === index))
}
...
function getFullNames(runners) {
  ...
  runners.map(runner => {
    roster.push(`${runner.last_name}, ${runner.first_name}`)
  })
  return roster
}
...
function firstNamesAllCaps(runners) {
  ...
  runners.map(runner => {
    roster.push(runner.first_name.toUpperCase())
  })
  return roster
}
